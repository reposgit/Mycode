...
class WC_Emails {
	...
	public function __construct() {
		...
		add_action( 'woocommerce_email_footer_custom', array( $this, 'email_footer_my' ) );
		...
		add_action( 'woocommerce_email_order_details_custom', array( $this, 'order_details_custom' ), 10, 4 );
        ...
        add_action( 'woocommerce_email_customer_details_custom', array( $this, 'customer_details_my' ), 10, 3 );
		add_action( 'woocommerce_email_customer_details_custom', array( $this, 'email_addresses_my' ), 20, 3 );
	...
	public function email_footer_my() {
		wc_get_template( 'emails/email-footer-custom.php' );
	}
	...
	public function order_details_custom( $order, $sent_to_admin = false, $plain_text = false, $email = '' ) {
		if ( $plain_text ) {
			wc_get_template(
				'emails/plain/email-order-details-custom.php', array(
					'order'         => $order,
					'sent_to_admin' => $sent_to_admin,
					'plain_text'    => $plain_text,
					'email'         => $email,
			...
			wc_get_template(
				'emails/email-order-details-custom.php', array(
					'order'         => $order,
					'sent_to_admin' => $sent_to_admin,
					'plain_text'    => $plain_text,
					'email'         => $email,
	...
	public function customer_details_my( $order, $sent_to_admin = false, $plain_text = false ) {
		if ( ! is_a( $order, 'WC_Order' ) ) {
			return;
		}
		...
		if ( ! empty( $fields ) ) {
			if ( $plain_text ) {
				...
				wc_get_template( 'emails/email-customer-details-my.php', array( 'fields' => $fields ) );
			}
		}
	}
	 ...
	 * @param bool     $plain_text    If is plain text email.
	 */
	public function email_addresses_my( $order, $sent_to_admin = false, $plain_text = false ) {
		if ( ! is_a( $order, 'WC_Order' ) ) {
			return;
		}
		if ( $plain_text ) {
			...
			wc_get_template(
				'emails/email-addresses-my.php', array(
					'order'         => $order,
					'sent_to_admin' => $sent_to_admin,
				)
			);
